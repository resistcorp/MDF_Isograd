function main(){
	const [P, I] = ri();
	const edges = rl(I).map(l => ints(l)).map(([A,B]) => {return {A, B}});
	const g = makeGraph(edges, P, false);
	let best = P+1;
	const set = new Set();
	let num = 0;

	for(const path of pathsOfLength(g, 0, P, true)){
		eprintln(`path of length ${path.length} : ${path.set}`);
		// let nA = g.nodes[0];
		// let max = 0;
		// for(const node of iter(path)){
		// 	if(node == 0)continue;
		// 	const nB = g.nodes[path.node];
		// 	set.clear();
		// 	nA.neighbours.forEach(v => set.add(v));

		// 	const common = nB.neighbours.filter(v => set.has(v)).length;
		// 	max = Math.max(max, common);
		// 	nA = nB;
		// }
		// best = Math.min(max, best);
		num++;
	}
	eprintln(`saw ${num} paths`);
	// eprintln(`${I} (${edges.length})edges : ${edges.map(({A, B})=> `${A}--${B}`)}`)
	// for(const node of g.nodes){
	// 	set.clear();
	// 	let isComplete = true; 
	// 	const ns = node.neighbours ?? [];
	// 	for(let i = 0; i < ns.length; i++){
	// 		const nI = g.nodes[ns[i]];
	// 		for(let j = 0; j < i; j++){
	// 			// const nJ = graph.nodes[ns[j]];
	// 			isComplete &&= nI.neighbours.includes(ns[j]);
	// 		}
	// 	}
	// 	eprintln(`node ${node.index} has ${ns.length} neighbours ${ns}`)
  // 	if(isComplete){
	// 		eprintln(`it's a complete graph  with ${ns.length+1} nodes!`)

	// 		max = Math.max(max, ns.length+1);
	// 	}
	// }
	print(best)
}
