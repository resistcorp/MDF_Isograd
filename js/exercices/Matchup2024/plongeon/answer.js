function main(){
	const seconds = ri();
	const figures = rpl().map(line => {
		const [time, score] = ints(line)
		return {time, score};
	});

	const score = calcBest(figures, seconds, 0, figures.map(_=>new Map()));
	print(score);
}

function calcBest(figures, seconds, index, hashmaps){
	if(seconds <= 0 || index >= figures.length)
		return 0;
	let best = 0;//calcBest(figures, seconds, index + 1, hashmaps);
	let figure = figures[index];
	let hashmap = hashmaps[index];
	if(hashmap.has(seconds))
		return hashmap.get(seconds);
	let {time, score} = figure;
	let malus = 0;
  const indent = "| ".repeat(index);
	// eprintln(`${indent} at ${index} (${score}:${time}s) in ${seconds}`);
	for(let times = 0; (times * time) <= seconds; times++){
		let value = score * times - malus;
		if(value < 0)
			break;
		// eprintln(`${indent} trying ${times} times (${time * times}s, ${value} points)`)
		let then = calcBest(figures, seconds - (time * times), index + 1, hashmaps);
		// let thenPs = then? then.points : 0;
		let points = value + then;
		if(best <= points)
			best = points;//{points, times, value/*, then*/, index}
		malus +=times;
	}
	hashmap.set(seconds, best);

	// let curr = {...best};
	// const all = [];
	// while(curr){
	// 	//if(curr.times)
	// 		all.push(`${curr.times}x${curr.index}(${curr.value})`);
	// 	curr = curr.then;
	// }
	// eprintln(`${indent}${index}(${score},${time}s) in ${seconds}s : ${best.points}pts ${all.join(" then ")}`);

	// eprintln(`${indent}best for ${index} (${score}:${time}s) in ${seconds} seconds : ${best.times} times for ${best.value} points (so ${best.points})`);
